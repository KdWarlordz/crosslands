...
Assets.RegisterSprites(BuildingTexture, LEVEL_BUILDING_TILE.DEFAULT)
// Assets.RegisterSprites(BuildingTexture, LEVEL_BUILDING_TILE.SMALL)
// Assets.RegisterSprites(BuildingTexture, LEVEL_BUILDING_TILE.LARGE)
// Assets.RegisterSprites(BuildingTexture, LEVEL_BUILDING_TILE.MEDIUM)
// Assets.RegisterSprites(BuildingTexture, LEVEL_BUILDING_TILE.XLARGE)
  ...
  implements IBuilding, ITile
{
  ...
  constructor(
    ...
    const tileConfig = LEVEL_BUILDING_TILE[size]
    ...
    this.setOrigin(0.5, tileConfig.origin)
    this.setDisplaySize(tileConfig.width, tileConfig.height)
  ...
  private updateTileCost() {
    const cost =
      ...
      Number(
        (
          this.live.maxHealth * DIFFICULTY.BUILDING_TILE_COST_MULTIPLIER
        ).toFixed(1)
      )
  ...
  private getRepairCost() {
    ...
    return Math.ceil(
      cost * damaged * DIFFICULTY.BUILDING_REPAIR_COST_MULTIPLIER
    )
  }
  ...
  private updateIndicatorsPosition() {
    ...
    this.indicators.setPosition(
      position.x - LEVEL_BUILDING_TILE[this.size].width / 4,
      position.y - 3
    )
  }
  ...
  private onDamage() {
    ...
    this.scene.fx.playSound([BuildingAudio.DAMAGE_1, BuildingAudio.DAMAGE_2], {
      limit: 1
    })
  ...
  public getTopEdgePosition() {
    return {
      x: this.x,
      y: this.y - LEVEL_BUILDING_TILE[this.size].height * 0.5
    }
  }
  ...
  protected addAlertIcon() {
    ...
    this.alertIcon = this.scene.add.image(
      position.x,
      position.y,
      BuildingIcon.ALERT
    )
  ...
  private addBuildBar() {
    ...
    this.buildBar = new Indicator(this, {
      size: LEVEL_BUILDING_TILE[this.size].width / 2,
      color: 0xffffff
    })
