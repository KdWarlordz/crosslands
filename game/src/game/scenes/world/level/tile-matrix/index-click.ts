...
export class TileMatrix implements ITileMatrix {
  ...
  public putTile(tile: ITile, position: TilePosition, destroyable = true) {
    ...
    // Ensure the tile has an interactive area matching its graphical representation
    tile.setInteractive(
      new Phaser.Geom.Rectangle(0, 0, tile.width, tile.height),
      Phaser.Geom.Rectangle.Contains
    )
    tile.on('pointerdown', () => this.handleTileClick(tile, position))
  ...
  private handleTileClick(tile: ITile, position: TilePosition) {
    console.log('Tile clicked:', position, tile)
    // Add custom logic for handling tile click event here
  }
}
