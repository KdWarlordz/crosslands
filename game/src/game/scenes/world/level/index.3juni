...
export class Level extends TileMatrix implements ILevel {
  ...
  private nonSelectablePositions: Set<string> = new Set()
  private canSelectScenery: boolean

  constructor(
    scene: IWorld,
    { planet, seed, showCount = true, canSelectScenery = false }: LevelData
  ) {
    super(LEVEL_MAP_SIZE, LEVEL_MAP_MAX_HEIGHT)

    this.scene = scene
    this.planet = planet ?? LevelPlanet.EARTH
    this.canSelectScenery = canSelectScenery
  ...
  // Add the getSelectedPosition method
  public getSelectedPosition(): PositionAtMatrix {
    if (this.selectedTiles.length === 0) {
      return { x: 0, y: 0 } // Default position if no tiles are selected
    }

    // Return the first selected tile as the position for building placement
  ...
  private addScenery() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (this.isFreePoint(tilePosition)) {
        ...
        // Mark the position as non-selectable
        this.nonSelectablePositions.add(`${tilePosition.x},${tilePosition.y}`)
      }
    }
  }
  ...
  private handleTileSelection(
    ...
    const tileKey = `${clickedTile.x},${clickedTile.y}`
    if (!this.canSelectScenery && this.nonSelectablePositions.has(tileKey)) {
      console.log('Cannot select this tile. It contains scenery.')
      return
    }
  ...
  private drawSelectionRectangle(tiles: PositionAtMatrix[]) {
    ...
    const regions = this.identifyRegions(tiles)
    this.colorRegionsWithShapes(regions)
  }

  private identifyRegions(tiles: PositionAtMatrix[]): PositionAtMatrix[][] {
    const floodFill = (
  ...
  private colorRegionsWithShapes(regions: PositionAtMatrix[][]) {
    const shapes = [
      { size: 64, color: 0x800080, width: 8, height: 8 },
      { size: 32, color: 0xffa500, width: 8, height: 4 },
      { size: 32, color: 0xffa500, width: 4, height: 8 },
      { size: 16, color: 0x0000ff, width: 4, height: 4 },
      { size: 8, color: 0xffff00, width: 4, height: 2 },
      { size: 8, color: 0xffff00, width: 2, height: 4 },
      { size: 4, color: 0xff00ff, width: 2, height: 2 },
      { size: 1, color: 0x00ffff, width: 1, height: 1 }
    ]

    regions.forEach((region) => {
      this.colorRegionRecursively(region, shapes)
    })
  }

  private colorRegionRecursively(region: PositionAtMatrix[], shapes: any[]) {
    let remainingTiles = [...region]

    shapes.forEach((shape) => {
      while (remainingTiles.length >= shape.size) {
        const subRegion = this.extractShape(
          remainingTiles,
          shape.width,
          shape.height
        )
        if (subRegion.length === shape.size) {
          this.colorRegion(subRegion, shape.color)
          remainingTiles = remainingTiles.filter(
            (tile) => !subRegion.some((t) => t.x === tile.x && t.y === tile.y)
          )
        } else {
          break
        }
    ...
    // Handle remaining individual tiles if any
    if (remainingTiles.length > 0) {
      const singleTileShape = shapes.find((shape) => shape.size === 1)
      if (singleTileShape) {
        this.colorRegion(remainingTiles, singleTileShape.color)
      }
    }
  }

  private extractShape(
    tiles: PositionAtMatrix[],
    width: number,
    height: number
  ): PositionAtMatrix[] {
    const shape: PositionAtMatrix[] = []
    ...
    while (stack.length > 0 && shape.length < width * height) {
      const { x, y } = stack.pop()!
      const key = `${x},${y}`
      ...
      if (
        ...
        for (let direction of directions) {
          ...
          if (
            !visited.has(nextKey) &&
            tiles.some((tile) => tile.x === nextX && tile.y === nextY)
          ) {
    ...
    // Verify if the shape is valid
    const xValues = shape.map((tile) => tile.x)
    const yValues = shape.map((tile) => tile.y)
    ...
    const validShape =
      (maxX - minX + 1 === width && maxY - minY + 1 === height) ||
      (maxX - minX + 1 === height && maxY - minY + 1 === width)

    return validShape ? shape : []
  }

  private colorRegion(region: PositionAtMatrix[], color: number) {
    region.forEach((tile, index) => {
      ...
      this.selectionRectangle.fillStyle(isInvalid ? 0xff0000 : color, 0.5)
