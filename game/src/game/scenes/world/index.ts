...
export class World extends Scene implements IWorld {
  ...
  public showHint(hint: WorldHint) {
    const id = hint.unique ? Utils.HashString(hint.label) : uuidv4()
  ...
  private addCrystals() {
    ...
    const getMaxCount = () =>
      progressionLinear({
        defaultValue:
          DIFFICULTY.CRYSTAL_COUNT / this.game.getDifficultyMultiplier(),
    ...
    this.wave.on(WaveEvent.COMPLETE, () => {
      const newCount =
        getMaxCount() - this.getEntitiesGroup(EntityType.CRYSTAL).getTotalUsed()
  ...
  private loadSavePayload(data: WorldSavePayload) {
    data.buildings.forEach((buildingData) => {
      try {
        ...
        // @ts-ignore
        const variant =
          buildingData.variant === 'TOWER_ELECTRO'
        ...
        console.warn(
          `Failed to load '${buildingData.variant}' building`,
          error as TypeError
        )
      }
    })
