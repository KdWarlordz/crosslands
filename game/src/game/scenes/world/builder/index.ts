...
import type { IBuildingFactory } from '../entities/building/factory/types'
...
export class Builder extends Phaser.Events.EventEmitter implements IBuilder {
  ...
  public get supposedPosition() {
    return this._supposedPosition
  }

  private set supposedPosition(v) {
    this._supposedPosition = v
  }
  ...
  public setBuildingVariant(variant: BuildingVariant) {
    ...
    if (!this.isBuildingAllowByWave(variant)) {
      this.scene.game.screen.failure('BUILDING_WILL_BE_AVAILABLE', [
        BuildingInstance.AllowByWave
      ])
    ...
    if (this.isBuildingLimitReached(variant)) {
      this.scene.game.screen.failure('BUILDING_LIMIT_REACHED', [
        phrase(`BUILDING_NAME_${variant}`)
      ])
  ...
  private open() {
    ...
    if (!this.scene.game.isDesktop()) {
      this.supposedPosition =
        this.scene.level.getFreeAdjacentTiles(
  ...
  private isAllowBuild() {
    ...
    const isFreeFromSprite = targets.every((npc) =>
      npc
        ...
        .every((point) => !isPositionsEqual(positionAtMatrix, point))
    )
  ...
  private build() {
    ...
    if (this.isBuildingLimitReached(this.variant)) {
      this.scene.game.screen.failure('BUILDING_LIMIT_REACHED', [
        phrase(`BUILDING_NAME_${this.variant}`)
      ])
    ...
    this.createBuilding({
      ...
      size: this.getBuildingSize(BUILDINGS[this.variant])
    })
  ...
  private createBuildPreview() {
    ...
    const BuildingInstance = BUILDINGS[this.variant]
    const sizeConfig = this.variant
      ? LEVEL_BUILDING_TILE[this.getBuildingSize(BUILDINGS[this.variant])]
      : LEVEL_BUILDING_TILE.DEFAULT

    this.buildPreview = this.scene.add.image(0, 0, BuildingInstance.Texture)
    this.buildPreview.setOrigin(0.5, sizeConfig.origin)
  ...
  private createBuildActionRadius() {
    ...
    this.buildActionRadius = this.scene.add.ellipse(
      0,
      0,
      d,
      d * LEVEL_MAP_PERSPECTIVE
    )
  ...
  private updateBuildInstance() {
    ...
    if (this.buildControls) {
      const confirmButton =
        this.buildControls.getAt<Phaser.GameObjects.Image>(0)
      ...
      this.buildControls.setPosition(
        positionAtWorld.x,
        positionAtWorld.y + sizeConfig.height
      )
  ...
  public isBuildingAllowByTutorial(variant: BuildingVariant) {
    ...
    const restrictions: {
      ...
      {
        step: TutorialStep.BUILD_GENERATOR_SECOND,
        variant: BuildingVariant.GENERATOR
      },
      {
        step: TutorialStep.BUILD_AMMUNITION,
    ...
    const restriction = restrictions.find((item) =>
      Tutorial.IsInProgress(item.step)
    )
  ...
  private getBuildingSize(
    BuildingInstance: IBuildingFactory
  ): keyof typeof LEVEL_BUILDING_TILE {
    switch (BuildingInstance.Texture) {
      case BuildingTexture.TOWER_FIRE:
      case BuildingTexture.TOWER_FROZEN:
      case BuildingTexture.TOWER_LAZER:
      case BuildingTexture.TOWER_ELECTRO:
      case BuildingTexture.GENERATOR:
      case BuildingTexture.RADAR:
      case BuildingTexture.BOOSTER:
      case BuildingTexture.AMMUNITION:
      case BuildingTexture.WALL:
        return 'DEFAULT'
      default:
        return 'DEFAULT'
    }
