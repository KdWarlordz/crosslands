...
import { getMap, getActiveChainConfig } from '~lib/client/api'
...
export class Game extends Phaser.Game implements IGame {
  ...
  constructor() {
    ...
    this.init().then(() => {
      ...
      this.events.on(
        `${GameEvent.UPDATE_SETTINGS}.${GameSettings.AUDIO}`,
        (enabled: boolean) => {
      ...
      this.events.on(
        `${GameEvent.UPDATE_SETTINGS}.${GameSettings.TUTORIAL}`,
        (enabled: boolean) => {
          if (enabled) {
            Tutorial.Enable()
          } else {
  ...
  public async init() {
    try {
      const activeChainConfig = await getActiveChainConfig()
      if (!activeChainConfig) {
        throw new Error('Failed to load active chain config')
      }
      const { chain, world, id } = activeChainConfig

      // there is some kind of race condition, where the first load leads to a black screen
      // when map is not available. I suspect its because of writing to local storage
      // and loading but not done yet.

      // also since we already store the API response in local storage, we can just load it
      // and avoid storing it again

      const map = await getMap(chain, world, id)
      await this.storeMap(map)
    } catch (error) {
      console.error(error)
    }
  }

  public async storeMap(mapData: any) {
    try {
      const stringValue = JSON.stringify(mapData)
      await localStorage.setItem('map', stringValue) // Note: localStorage API is synchronous, so no need for await here
      console.log(`Data stored under key: map`)
    } catch (error) {
      console.error('Error writing to local map(s) storage', error)
    }
  }
  ...
  public pauseGame() {
    ...
    this.scene.systemScene.scene.launch(GameScene.MENU, {
      defaultPage: this.isDesktop() ? MenuPage.CONTROLS : MenuPage.ABOUT_GAME
    })
  }
